...
const { getEmployeeRoles, deleteEmployeeRoles } = actions(['employeeRoles'], { get: true, remove: true });
// console.log(action, 'list');
function openNew() {
    router.push({ name: 'employee_roles-create', params: { id: 'new' } });
}
...
function deleteProduct() {
    ...
    deleteEmployeeRoles(crop.value.id)
        .then((res) => {
            crop.value = {};
            toast.add({ severity: 'success', summary: t('employee_roles'), detail: t('employee_roles') + ' ' + t('delete'), life: 3000 });
...
function getCropsList() {
    ...
    return getEmployeeRoles(filters)
        .then((res) => {
