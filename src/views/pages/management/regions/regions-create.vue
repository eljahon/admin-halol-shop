...
const { postRegions, putRegions, getByIdRegions } = actions(['regions'], { get: true, post: true, put: true, getById: true });
...
if (route.params.id !== 'new') {
    isUpdate.value = true;
    getByIdRegions({ id: route.params.id, query: { populate: '*' } })
        .then((res) => {
...
async function handleSubmitFrom(values) {
    ...
    const data = values.id
        ...
        : await postRegions({ data: values.data });
    if (data) {
        router.go(-1);
