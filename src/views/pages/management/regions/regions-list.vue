...
const { getRegions, deleteRegions } = actions(['regions'], { get: true, remove: true });
function openNew() {
    router.push({ name: 'regions-create', params: { id: 'new' } });
}
        ...
        <div class="card">
            ...
            <DataTable ref="dt" :value="crops" dataKey="id" :loading="isLoading">
                ...
                <Column :header="$t('actions')" :frozen="actions" style="min-width: 8rem;display: flex; justify-content: end">
                    <template #body="{ data }">
                        <Button icon="pi pi-pencil" outlined rounded class="mr-2" @click="router.push({ name: 'regions-create', params: { id: data.id } })" />
