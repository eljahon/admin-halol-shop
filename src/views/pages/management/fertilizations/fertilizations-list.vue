...
const dt = ref();
const fertilizations = ref();
const fertilization = ref();
const search = ref(route.query?.search ?? undefined);
const deleteFertilizationsDialog = ref(false);
const isLoading = ref(false);

const meta = ref({});

const { getFertilizations, deleteFertilizations } = actions(['fertilizations'], { get: true, remove: true });
function openNew() {
    router.push({ name: 'fertilizations-create', params: { id: 'new' } });
}

function confirmDeleteProduct(prod) {
    fertilization.value = prod;
    deleteFertilizationsDialog.value = true;
}

function deleteProduct() {
    deleteFertilizationsDialog.value = false;
    isLoading.value = true;
    deleteFertilizations(fertilization.value.id)
        .then((res) => {
            fertilization.value = {};
            toast.add({ severity: 'success', summary: 'Successful', detail: t('fertilizations') + ' ' + t('delete'), life: 3000 });
            isLoading.value = false;
            getFertilizationsList();
        })
        .catch((err) => {
...
function getFertilizationsList() {
    isLoading.value = true;
    ...
    getFertilizations(filters)
        .then((res) => {
            fertilizations.value = res.data;
            meta.value = res.meta;
            isLoading.value = false;
        })
        .catch((err) => {
...
<template>
    <div>
        ...
        <div class="card">
            <div class="flex gap-2 justify-between">
                <div>
                    <IconField>
                        ...
                        <InputText v-model="search" :placeholder="$t('crop-name')" @input="onSearch" />
                    </IconField>
                </div>
            ...
            <DataTable ref="dt" :value="fertilizations" dataKey="id" :loading="isLoading">
                ...
                <Column field="fertilizer.name" :header="$t('fertilizer')" style="min-width: 10rem"></Column>
                ...
                <Column field="use_count" :header="$t('use_count')" style="min-width: 1rem"></Column>
                <Column field="method" :header="$t('method')" style="min-width: 10rem"></Column>
                <Column :header="$t('actions')" :frozen="actions" style="min-width: 8rem;">
                    <template #body="{ data }">
                        <Button icon="pi pi-pencil" outlined rounded class="mr-2" @click="router.push({ name: 'fertilizations-create', params: { id: data.id } })" />
        ...
        <Dialog v-model:visible="deleteFertilizationsDialog" :style="{ width: '450px' }" :modal="true">
            <template #header>
            ...
            <div class="flex items-center gap-4">
                ...
                <span v-if="fertilization"
                    >{{ $t('you-want-to-delete') }} <b>{{ fertilizations.name }}</b
                    >?</span
                >
            </div>
            <template #footer>
                <Button severity="Danger" outlined :label="$t('no')" icon="pi pi-times" text @click="deleteFertilizationsDialog = false" />
