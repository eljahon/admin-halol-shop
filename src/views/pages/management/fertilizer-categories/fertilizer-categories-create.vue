...
const { postFertilizerCategories, putFertilizerCategories, getByIdFertilizerCategories } = actions(['fertilizerCategories'], { get: true, post: true, put: true, getById: true });
...
if (route.params.id !== 'new') {
    isUpdate.value = true;
    getByIdFertilizerCategories({ id: route.params.id, query: { populate: '*' } })
        .then((res) => {
...
async function handleSubmitFrom(values) {
    ...
    const data = values.id
        ? await putFertilizerCategories({
              id: values.id,
              data: values.data
          })
        : await postFertilizerCategories({ data: values.data });
    if (data) {
        router.go(-1);
