...
const { postSeasons, putSeasons, getByIdSeasons } = actions(['seasons'], { get: true, post: true, put: true, getById: true });
...
if (route.params.id !== 'new') {
    isUpdate.value = true;
    getByIdSeasons({ id: route.params.id, query: { populate: '*' } })
        .then((res) => {
...
async function handleSubmitFrom(values) {
    ...
    const data = values.id
        ...
        : await postSeasons({ data: values.data });
    if (data) {
        router.go(-1);
        toast.add({ severity: 'success', summary: t('activity_types'), detail: t('seasons') + ' ' + values.id ? t('update') : t('create'), life: 3000 });
