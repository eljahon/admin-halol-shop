...
const { getAreas, deleteAreas } = actions(['areas'], { get: true, remove: true });
function openNew() {
    router.push({ name: 'areas-create', params: { id: 'new' } });
}
...
function getAreasList() {
    ...
    const filters = {
        ...
        filters:{ name:{$containsi: _query.search?? undefined} }
    };
    getAreas(filters)
        .then((res) => {
        ...
        <div class="card">
            ...
            <DataTable ref="dt" :value="crops" dataKey="id" :loading="isLoading">
                ...
                <Column :header="$t('actions')" :frozen="actions" style="min-width: 12rem; text-align: end">
                    <template #body="{ data }">
                        <Button icon="pi pi-pencil" outlined rounded class="mr-2" @click="router.push({ name: 'areas-create', params: { id: data.id } })" />
