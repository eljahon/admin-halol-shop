...
const { postDiseasesCategories, putDiseasesCategories, getByIdDiseasesCategories, getDiseasesCategories } = actions(['diseasesCategoriesTree', 'diseasesCategories'], { get: true, post: true, put: true, getById: true });
...
if (route.params.id !== 'new') {
    isUpdate.value = true;
    getByIdDiseasesCategories({ id: route.params.id, query: { populate: '*' } })
        .then((res) => {
...
async function handleSubmitFrom(values) {
    ...
    const data = values.id
        ...
        : await postDiseasesCategories({ data: { ...values.data, icon: icon.value.id } });
    if (data) {
        router.go(-1);
