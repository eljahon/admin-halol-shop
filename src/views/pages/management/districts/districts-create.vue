...
const { postDistricts, putDistricts, getByIdDistricts, getRegions } = actions(['districts', 'regions'], { get: true, post: true, put: true, getById: true });
...
if (route.params.id !== 'new') {
    isUpdate.value = true;
    getByIdDistricts({ id: route.params.id, query: { populate: '*' } })
        .then((res) => {
...
async function handleSubmitFrom(values) {
    ...
    const data = values.id
        ...
        : await postDistricts({ data: values.data });
    if (data) {
        router.go(-1);
