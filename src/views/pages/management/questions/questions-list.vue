...
const { getQuestions, deleteQuestions } = actions(['questions'], { get: true, remove: true });
function openNew() {
    router.push({ name: 'questions-create', params: { id: 'new' } });
}
        ...
        <div class="card">
            ...
            <DataTable ref="dt" :value="crops" dataKey="id" :loading="isLoading">
                ...
                <Column :header="$t('actions')" :frozen="actions" style="min-width: 12rem; text-align: end">
                    <template #body="{ data }">
                        <Button icon="pi pi-pencil" outlined rounded class="mr-2" @click="router.push({ name: 'questions-create', params: { id: data.id } })" />
