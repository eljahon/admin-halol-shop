...
const { getDrugCategories, deleteDrugCategories } = actions(['drugCategories'], { get: true, remove: true });
function openNew() {
    router.push({ name: 'drugCategories-create', params: { id: 'new' } });
}
...
function deleteProduct() {
    ...
    deleteDrugCategories(crop.value.id)
        .then((res) => {
            crop.value = {};
            toast.add({ severity: 'success', summary: 'Successful', detail: t('DrugCategories') + ' ' + t('delete'), life: 3000 });
        ...
        <div class="card">
            ...
            <DataTable ref="dt" :value="diseasesCategories" dataKey="id" :loading="isLoading">
                ...
                <Column :header="$t('actions')" :frozen="actions" style="min-width: 12rem; text-align: end">
                    <template #body="{ data }">
                        <Button icon="pi pi-pencil" outlined rounded class="mr-2" @click="router.push({ name: 'drugCategories-create', params: { id: data.id } })" />
