...
const { handleSubmit, defineField, resetForm, errors, setFieldValue, values, isSubmitting } = useForm({
    validationSchema: yup.object(
        props.feilds.reduce((acc, el) => {
            acc[el.label] = el.schema.required ? yup[el.schema.type]().required(t(`${el.label}`) + ' ' + t('required-fild')) : '';
            return acc;
        }, {})
    ),
            ...
            <div v-for="(item, index) in feilds" :key="index" :class="[itemClass]">
                ...
                <div v-else>
                    <div :class="[item.prop.class]">
                        ...
                        <component :invalid="!!errors[item.label]" class="w-full" v-bind="item.prop" :is="componentList[item.renderElement]" v-model="defineFieldForm[index][0].value" :type="item.prop?.type" :placeholder="$t(`${item.label}`)">
                        </component>
                    </div>
